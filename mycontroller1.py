...
def write_register(sw, p4info_helper, register_name, index, value):
    ...
    register = next(r for r in p4info_helper.p4info.registers if r.preamble.name == register_name)
    register_id = register.preamble.id
    if register.type_spec.HasField('bitstring') and register.type_spec.bitstring.HasField('bit'):
        register_size = register.type_spec.bitstring.bit.bitwidth
        print(f"Writing to register {register_name} (ID: {register_id}, Size: {register_size} bits)")
    else:
        print(f"Register {register_name} type_spec is not bitstring.bit: {register.type_spec}")
        if register_name.endswith("hyperperiod_duration_reg") or register_name.endswith("last_hyperperiod_reg"):
            ...
            print(f"Unknown size for register {register_name}")
            return
        print(f"Using default size: {register_size} bits")
    
    register_entry.register_id = register_id
    register_entry.index.index = index
    try:
        bitstring = value.to_bytes((register_size + 7) // 8, byteorder='big')
        register_entry.data.bitstring = bitstring
        print(f"Converted value to bitstring: {bitstring.hex()}")
    except OverflowError as e:
        print(f"Error converting value {value} for register {register_name}: {e}")
        return

    try:
        sw.client_stub.Write(request)
        print(f"Successfully wrote {value} to {register_name} at index {index}")
    except grpc.RpcError as e:
        print(f"Failed to write to {register_name}: {e}")

def writeRegisters(sw, p4info_helper):
    """
    ...
    ts_micro &= (1 << 48) - 1  # Mask to 48 bits

    # Write register values
    # Stream Gate 1 configuration
    write_register(sw, p4info_helper, "IngressImpl.psfp_c.hyperperiod_duration_reg", 1, 20000000) # Hyperperiod duration for stream Gate 1 is 10 seconds (in microseconds)
    write_register(sw, p4info_helper, "IngressImpl.psfp_c.last_hyperperiod_reg", 1, ts_micro)
    write_register(sw, p4info_helper, "IngressImpl.psfp_c.hyperperiod_done_reg", 1, 0)

    # Stream Gate 2 configuration
    write_register(sw, p4info_helper, "IngressImpl.psfp_c.hyperperiod_duration_reg", 2, 16000000) # Hyperperiod duration for stream Gate 2 is 8 seconds (in microseconds)
    write_register(sw, p4info_helper, "IngressImpl.psfp_c.last_hyperperiod_reg", 2, ts_micro)
    write_register(sw, p4info_helper, "IngressImpl.psfp_c.hyperperiod_done_reg", 2, 0)

    # Stream Gate 3 configuration
    write_register(sw, p4info_helper, "IngressImpl.psfp_c.hyperperiod_duration_reg", 3, 26000000) # Hyperperiod duration for stream Gate 3 is 13 seconds (in microseconds)
    write_register(sw, p4info_helper, "IngressImpl.psfp_c.last_hyperperiod_reg", 3, ts_micro)
    write_register(sw, p4info_helper, "IngressImpl.psfp_c.hyperperiod_done_reg", 3, 0)

    # Stream Gate 4 configuration
    write_register(sw, p4info_helper, "IngressImpl.psfp_c.hyperperiod_duration_reg", 4, 20000000) # Hyperperiod duration for stream Gate 4 is 10 seconds (in microseconds)
    write_register(sw, p4info_helper, "IngressImpl.psfp_c.last_hyperperiod_reg", 4, ts_micro)
    write_register(sw, p4info_helper, "IngressImpl.psfp_c.hyperperiod_done_reg", 4, 0)
...
def main(p4info_file_path, bmv2_file_path):
    ...
    try:
        ...
        # Write register values
        writeRegisters(s1, p4info_helper)
