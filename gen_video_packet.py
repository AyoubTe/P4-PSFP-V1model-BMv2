...
def main():
    ...
    vlan_id = 1  # VLAN ID for video traffic
    pcp = 5       # Priority Code Point for video (high priority)
    iface = get_if()

    print(f"Sending video packets on interface {iface} to {addr} with VLAN ID {vlan_id}, PCP {pcp}")
    pkt = (Ether(src=get_if_hwaddr(iface), dst='08:00:00:00:02:22') /
           ...
           (sys.argv[2] * 100))  # Larger payload for video
    pkt.show2()
